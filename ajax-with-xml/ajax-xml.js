 ...
 * other functions. More on that later.
 */
window.onload = function() {
    ...
    function showCustomer(customerName) {
         ...
         * organized. They have direct access to variables defined in their
         * containing functions, unless a variable with the same name exists
         * in a more local scope. Since the functions below aren't useful
         * outside showCustomer(), there's no point in having them be global.
         *
         * This one traverses a nasty graph (the DOM) that represents the XML
         * document and extracts the value of the first tag named fieldName.
         */
        function getResponseValue(fieldName) {
            return xmlDoc.getElementsByTagName(fieldName)[0]
